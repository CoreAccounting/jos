...
namespace templateProject.Repository
{
    public class PlantRepository : GenericRepository<MPlantModel>
    {
        ...
        public ResultStatusModel Plant(MPlantModel item, string mode, out string ID)
        {
            ...
            SqlParameter[] sqlParams =
            {
                ...
                new SqlParameter("PlantDesc", string.IsNullOrEmpty(item.PlantDesc) ? (object) DBNull.Value : item.PlantDesc),
            ...
            List<ResultStatusModel> result =
                Db.Database.SqlQuery<ResultStatusModel>(
                                                "exec sp_CUD_MPlant " +
                                                "@PlantID, @PlantName, @PlantDesc, @IsDeleted" +
       ...
       )
        {
            SqlParameter[] sqlParams =
            {
                ...
                new SqlParameter("PlantDesc", string.IsNullOrEmpty(_PlantDesc) ? (Object) DBNull.Value : _PlantDesc),
            ...
            List<MPlantModel> result =
                Db.Database.SqlQuery<MPlantModel>(
                                                @"exec sp_Lookup_MPlantList " +
                                                "@PlantID, @PlantName, @PlantDesc, @IsDeleted,@PageSize"
                                            , sqlParams).ToList();
